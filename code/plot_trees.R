...
#dOTU_se <- read.delim("data/neeat_discarded_clusters_se.tsv" , header = FALSE)
#dOTU_mg <- read.delim("data/neeat_discarded_clusters_mg.tsv" , header = FALSE)
...
# --------------- Get sample IDs 
se_meta <- fread("data/Sweden/CO1_sequencing_metadata_SE_v2.tsv")
mg_meta <- fread("data/Madagascar/CO1_sequencing_metadata_MG_v2.tsv")
...
swe_clusters <- fread("cleaned_noise_filtered_cluster_taxonomy_SE.tsv") |>
  _[representative == 1 ,] |> 
  _[Phylum == "Arthropoda" ,] |> 
...
mg_clusters  <- fread("cleaned_noise_filtered_cluster_taxonomy_MG.tsv") |>
  _[representative == 1 ,] |> 
  _[Phylum == "Arthropoda" ,] |> 
  _[!str_detect(Family , "unclassified*|_X*"),] |> 
  _[!str_detect(Order , "_X*"),] |> 
...
# ------------ Get read numbers 
swe_counts   <- fread("cleaned_noise_filtered_cluster_counts_SE.tsv" )
mg_counts    <- fread( "cleaned_noise_filtered_cluster_counts_MG.tsv" )
...
# Select clades: Arachnida, Myriapoda, Insecta (Pterygota), Collembola, Crustacea
malaise_clades_se <- c(2, 4, 6,11,16)
malaise_clade_labels_se <- malaise_tree_se$phylo$node.label[malaise_clades_se]
malaise_clade_labels_se[5] <- "Insecta" # Rename pterygota to insecta
